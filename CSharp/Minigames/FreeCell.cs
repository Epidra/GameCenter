...
namespace GameCenter {
    class FreeCell : Ghost {

        Vector2 selector_card = new Vector2(-1, -1);
        Vector2 selector_pos;

        List<Vector2>[] cards_field = new List<Vector2>[8];
        List<Vector2>[] cards_finish = new List<Vector2>[4];
        Vector2[] cards_freecell = new Vector2[4];

        public FreeCell(string _id, ContentManager _content, ShopKeeper _Shopkeeper, FileManager _Filemanager, JukeBox _Jukebox, float screenX, float screenY) : base(_id, _content, _Shopkeeper, _Filemanager, _Jukebox, screenX, screenY) {
            id = _id;
            CM = _content;
            SK = _Shopkeeper;
            FM = _Filemanager;
        ...
        public override void Start2() {
            ...
            cards_field[0] = new List<Vector2>(); cards_field[0].AddRange(deck.GetRange(0, 7)); deck.RemoveRange(0, 7);
            cards_field[1] = new List<Vector2>(); cards_field[1].AddRange(deck.GetRange(0, 7)); deck.RemoveRange(0, 7);
            cards_field[2] = new List<Vector2>(); cards_field[2].AddRange(deck.GetRange(0, 7)); deck.RemoveRange(0, 7);
            cards_field[3] = new List<Vector2>(); cards_field[3].AddRange(deck.GetRange(0, 7)); deck.RemoveRange(0, 7);
            cards_field[4] = new List<Vector2>(); cards_field[4].AddRange(deck.GetRange(0, 6)); deck.RemoveRange(0, 6);
            cards_field[5] = new List<Vector2>(); cards_field[5].AddRange(deck.GetRange(0, 6)); deck.RemoveRange(0, 6);
            cards_field[6] = new List<Vector2>(); cards_field[6].AddRange(deck.GetRange(0, 6)); deck.RemoveRange(0, 6);
            cards_field[7] = new List<Vector2>(); cards_field[7].AddRange(deck.GetRange(0, 6)); deck.RemoveRange(0, 6);
            ...
            cards_freecell[0] = new Vector2(-1, -1);
            cards_freecell[1] = new Vector2(-1, -1);
            cards_freecell[2] = new Vector2(-1, -1);
            cards_freecell[3] = new Vector2(-1, -1);
        ...
        public override string Update2() {
            ...
            if(ButtonPressed(GhostKey.button_ok_P1) == GhostState.pressed && pressed_event_touch) {
                ...
                if(Collision_Button(false, new Rectangle((int)SK.Position_DisplayEdge().X + 35 * 1, (int)SK.Position_DisplayEdge().Y + 48 * 1, 35*2, 48*2))) { Free_Cell(0); }
                if(Collision_Button(false, new Rectangle((int)SK.Position_DisplayEdge().X + 35 * 3, (int)SK.Position_DisplayEdge().Y + 48 * 1, 35*2, 48*2))) { Free_Cell(1); }
                if(Collision_Button(false, new Rectangle((int)SK.Position_DisplayEdge().X + 35 * 5, (int)SK.Position_DisplayEdge().Y + 48 * 1, 35*2, 48*2))) { Free_Cell(2); }
                if(Collision_Button(false, new Rectangle((int)SK.Position_DisplayEdge().X + 35 * 7, (int)SK.Position_DisplayEdge().Y + 48 * 1, 35*2, 48*2))) { Free_Cell(3); }

                if(Collision_Button(false, new Rectangle((int)SK.Position_DisplayEdge().X + 35 * 11, (int)SK.Position_DisplayEdge().Y + 48 * 1, 35*2, 48*2))) { TouchFinish(0); }
                if(Collision_Button(false, new Rectangle((int)SK.Position_DisplayEdge().X + 35 * 13, (int)SK.Position_DisplayEdge().Y + 48 * 1, 35*2, 48*2))) { TouchFinish(1); }
                if(Collision_Button(false, new Rectangle((int)SK.Position_DisplayEdge().X + 35 * 15, (int)SK.Position_DisplayEdge().Y + 48 * 1, 35*2, 48*2))) { TouchFinish(2); }
            ...
            if(ButtonPressed(GhostKey.button_ok_P1) == GhostState.pressed && !pressed_event_touch) {
                ...
                if(selector_pos == new Vector2( 1, 1)) Free_Cell(0);
                if(selector_pos == new Vector2( 3, 1)) Free_Cell(1);
                if(selector_pos == new Vector2( 5, 1)) Free_Cell(2);
                if(selector_pos == new Vector2( 7, 1)) Free_Cell(3);

                if(selector_pos == new Vector2(11, 1)) TouchFinish(0);
                if(selector_pos == new Vector2(13, 1)) TouchFinish(1);
                if(selector_pos == new Vector2(15, 1)) TouchFinish(2);
        ...
        private void Free_Cell(int cell) {
            if(selector_card == new Vector2(-1, -1)) {
                if(cards_freecell[cell] != new Vector2(-1, -1)) {
                    selector_card = new Vector2(cell, -2);
                }
            } else if(selector_card.Y >= 0) {
                if(cards_freecell[cell] == new Vector2(-1, -1)) {
                    if(selector_card.Y == cards_field[(int)selector_card.X].Count - 1) {
                        cards_freecell[cell] = cards_field[(int)selector_card.X][cards_field[(int)selector_card.X].Count - 1];
            ...
            if(selector_card != new Vector2(-1, -1)) {
                if(selector_card.Y == -2) { // Cell-to-Finish
                    if(cards_finish[slot].Count == 0) {
                        cards_finish[slot].Add(cards_freecell[(int)selector_card.X]);
                        cards_freecell[(int)selector_card.X] = new Vector2(-1, -1);
                        selector_card = new Vector2(-1, -1);
                    } else {
                        if((cards_freecell[(int)selector_card.X].X - 1 == cards_finish[slot][cards_finish[slot].Count - 1].X || (cards_freecell[(int)selector_card.X].X == 1 && cards_finish[slot][cards_finish[slot].Count - 1].X == 13)) && cards_finish[slot][cards_finish[slot].Count - 1].Y == cards_freecell[(int)selector_card.X].Y) {
                            cards_finish[slot].Add(cards_freecell[(int)selector_card.X]);
                            cards_freecell[(int)selector_card.X] = new Vector2(-1, -1);
                            selector_card = new Vector2(-1, -1);
                        }
        ...
        private bool MoveStack(int x, int y) {
            ...
            if(selector_card.Y != -2) { // Field-to-Field
                ...
                if(cards_field[x2].Count == 0) {
                    cards_field[x2].Add(cards_freecell[(int)selector_card.X]);
                    cards_freecell[(int)selector_card.X] = new Vector2(-1, -1);
                    selector_card = new Vector2(-1, -1);
                    return true;
                } else {
                    if((cards_freecell[(int)selector_card.X].X + 1 == cards_field[x2][y2].X || (cards_freecell[(int)selector_card.X].X == 13 && cards_field[x2][y2].X == 1)) && DifferentColors(cards_field[x2][y2].Y, cards_freecell[(int)selector_card.X].Y)) {
                        cards_field[x2].Add(cards_freecell[(int)selector_card.X]);
                        cards_freecell[(int)selector_card.X] = new Vector2(-1, -1);
                        selector_card = new Vector2(-1, -1);
                        return true;
        ...
        public override void Draw2() {
            ...
            if(cards_freecell[0] != new Vector2(-1, -1)) spriteBatch.Draw(SK.texture_spritesheet_cards, SK.Position_DisplayEdge() + new Vector2(35 * 1, 48 * 1), new Rectangle(1 + (int)(71 * cards_freecell[0].X), 1 + (int)(96 * cards_freecell[0].Y), 70, 95), selector_card == new Vector2(0, -2) ? Color.Gold : Color.White, 0.0f, new Vector2(0, 0), 1, SpriteEffects.None, 0.0f);
            if(cards_freecell[1] != new Vector2(-1, -1)) spriteBatch.Draw(SK.texture_spritesheet_cards, SK.Position_DisplayEdge() + new Vector2(35 * 3, 48 * 1), new Rectangle(1 + (int)(71 * cards_freecell[1].X), 1 + (int)(96 * cards_freecell[1].Y), 70, 95), selector_card == new Vector2(1, -2) ? Color.Gold : Color.White, 0.0f, new Vector2(0, 0), 1, SpriteEffects.None, 0.0f);
            if(cards_freecell[2] != new Vector2(-1, -1)) spriteBatch.Draw(SK.texture_spritesheet_cards, SK.Position_DisplayEdge() + new Vector2(35 * 5, 48 * 1), new Rectangle(1 + (int)(71 * cards_freecell[2].X), 1 + (int)(96 * cards_freecell[2].Y), 70, 95), selector_card == new Vector2(2, -2) ? Color.Gold : Color.White, 0.0f, new Vector2(0, 0), 1, SpriteEffects.None, 0.0f);
            if(cards_freecell[3] != new Vector2(-1, -1)) spriteBatch.Draw(SK.texture_spritesheet_cards, SK.Position_DisplayEdge() + new Vector2(35 * 7, 48 * 1), new Rectangle(1 + (int)(71 * cards_freecell[3].X), 1 + (int)(96 * cards_freecell[3].Y), 70, 95), selector_card == new Vector2(3, -2) ? Color.Gold : Color.White, 0.0f, new Vector2(0, 0), 1, SpriteEffects.None, 0.0f);

            if(cards_finish[0].Count > 0) spriteBatch.Draw(SK.texture_spritesheet_cards, SK.Position_DisplayEdge() + new Vector2(35 * 11, 48 * 1), new Rectangle(1 + (int)(71 * cards_finish[0][cards_finish[0].Count - 1].X), 1 + (int)(96 * cards_finish[0][cards_finish[0].Count - 1].Y), 70, 95), Color.White, 0.0f, new Vector2(0, 0), 1, SpriteEffects.None, 0.0f);
            if(cards_finish[1].Count > 0) spriteBatch.Draw(SK.texture_spritesheet_cards, SK.Position_DisplayEdge() + new Vector2(35 * 13, 48 * 1), new Rectangle(1 + (int)(71 * cards_finish[1][cards_finish[1].Count - 1].X), 1 + (int)(96 * cards_finish[1][cards_finish[1].Count - 1].Y), 70, 95), Color.White, 0.0f, new Vector2(0, 0), 1, SpriteEffects.None, 0.0f);
            if(cards_finish[2].Count > 0) spriteBatch.Draw(SK.texture_spritesheet_cards, SK.Position_DisplayEdge() + new Vector2(35 * 15, 48 * 1), new Rectangle(1 + (int)(71 * cards_finish[2][cards_finish[2].Count - 1].X), 1 + (int)(96 * cards_finish[2][cards_finish[2].Count - 1].Y), 70, 95), Color.White, 0.0f, new Vector2(0, 0), 1, SpriteEffects.None, 0.0f);
